...
//	console.info('write cookie: '+f + "=" + v + "; path=/");
	if(f=='windows'){
		v = 'VERSION_'+globalversion+':'+v;
	}
    document.cookie = f + "=" + v + "; path=/";
}
function cookieGrab(f) {

//	console.info("read cookie: "+f);
    var b = f + "=",
    ...
    for (var i = 0; i < d.length; i++) {
        ...
        if (c.indexOf(b) == 0) {
			...
			var content = c.substring(b.length, c.length);
//			console.info("read cookie: "+content);
			if(f=='windows')
			{
				var index = content.indexOf("VERSION_");
				if(index >-1){
					var versionnum = content.substr(index,content.indexOf(':'));
//					console.info(versionnum.split('_')[1]);
					if(parseInt(versionnum.split('_')[1])<globalversion)
						return ;
				}
			}			
...
//        console.info('F: '+ (F)? 'true':'false');
        if (stop) {
            ...
            function c(e, m) {        //call core.win     core.X      core.Y
                r = XY(e);
                if (C) r = C.fu(o, m, {
                        'X':r.X - oX,
                        'Y':r.Y - oY
            ...
            function f(e, m) {
                c(e, m);   // call c, update r: the coordination
                if (F) F(oXY, r, m, e);   // call F, deal whith event. oXY,r is the begin and cur coordination
                return (r);
            }
...
gui = {
    ...
    //* Check Box
    'check':{
        'build':function (v, c, r, h) {
            return ('<div id="' + c + '_check" class="check">' + ' <span>' + v + '</span><br>'
                + ' <div onclick="gui.check.click(this,\'' + r[0] + '\')"'
//              + (vars[r[0]] == 'true' ? 'class="cur"' : '') + '>'
                + (vars[r[0]] == 'true'?((h)?'class="cur hooker"':'class="cur"'):((h)?'class="hooker"':''))+'>'
                + vars[r[0]] + '</div>' + '</div>');
        ...
        'click':function (o, v) {
            function z(a, b, c, d) {
                ...
                o.innerHTML = c;
//                o.className = d;
                if(o.className.indexOf('hooker')>-1)
                {
                    o.className = d+' hooker';
                    hooker.fi();
                }
                else
                    o.className = d;
...
//            if (o.className == 'cur') z('block', 'none', 'false', '');
            if (o.className.indexOf('cur')>-1) z('block', 'none', 'false', '');
...
//                b += '<div class="' + (((vars[r[0]] == r[i] && vars.type != 'crop')
//                    || (vars[r[0]] == r[i] && vars.type == 'crop' && !crop.force(vars.crop))) ? ' cur' : '')
                b += '<div class="' + (((vars[r[0]] == r[i] && vars.type != 'crop')
                    || (vars[r[0]] == r[i] && vars.type == 'crop' && !crop.force(vars.crop))) ? ((h)?' cur hooker':' cur') : ((h)?'hooker':''))
                    + '" onclick="gui.radio.click(this,\'' + r[0] + '\')">' + r[i] + '</div><br>';
            return ('<div' + ((crop.force(vars.crop) && vars.type == 'crop') ? ' style="opacity: 0.6"' : '') + ' class="radio" id="'
                + c + '_radio"><span>' + v + '</span><br>' + b + '</div>');
        },
        'click':function (o, v) {
...
//            o.className = "cur";
            if($C("cur", o.parentNode)[0].className.indexOf('hooker')>-1)
                $C("cur", o.parentNode)[0].className = "hooker";
            else
                $C("cur", o.parentNode)[0].className = "";
            vars[v]=i;//2012年8月15日16:13:01
            if(o.className.indexOf('hooker')>-1)
            {
                o.className = ' cur hooker';
                hooker.fi();
            }
            else
                o.className = "cur";
    ...
    'menu':{
        'build':function (c, r) {
            ...
            for (var i in r) {
                var style = (i == 0) ? 'style="border-top: none;"'
                    : ((parseInt(i) + 1) == length ? 'style="border-bottom: none;"' : '');
                ...
                z += '<li onmousedown="gui.menu.toggle(this)" onmouseup="gui.menu.select(this)" onmouseover="gui.menu.okClose(this.parentNode.parentNode)" '
                    + style + ' ' + className + '>' + r[i] + '</li>';
            }
            return ('<div class="menuWrap" id="' + c + '_opt">'
                    + ' <div class="t"><div class="l"></div><div class="r"></div><div class="c"></div></div>'
                    + ' <div class="menuBox">' + '  <ul ' + position + '>'
                    + '   <li class="top"><div class="l"></div><div class="r"></div><div class="c"></div></li>'
                    + z + '   <li class="bottom"><div class="l"></div><div class="c"></div><div class="r"></div></li>'
                    + '  </ul>' + ' </div>'
        ...
        'fu':{
            ...
            'fi':function (c,o){
 //               console.info('c: '+ c +' o:'+o);
                gui.menu.fu.z(c,o);
                gui_filter.fileNumber = 1;
                stop = 1;
                var o = gui.Y;
                o.cur.fi = 1;
                o.prev.fi = null;
        ...
        'toggle':function (n) {
            ...
            if (p.opened != true) {
                ...
                o.prev = n;
                o.parent = p;
                var c = p.parentNode.id.replace('_opt', '');     //class menuWrap
                ...
                n.parentNode.className += 'opened'; // ul
...
//            console.info('gui.menu.doSelect call');
            ...
            for (var x = 0; x < o.length; x++) {
				if(!isfiltermenu&&(o[x].innerHTML == 'blur' ||o[x].innerHTML == 'stylize')){
					isfiltermenu = true;
				}
            ...
            gui.menu.fu[c](c, n, s);
			if(isfiltermenu){
				$('filter_placeholder').innerHTML='<div></div>';
				gui_filter.resize(180);
			}
    ...
    'Y':{
        ...
        'kontrol':function (i, n) {
            var o = gui.Y,
                r = o.r[i],
                H = o.height(i);
            return ('<div id="' + i + 'Kontrol" onmousedown="gui.Y.slide_fu(event,\'' + i + '\')" class="slideY" style="top: '
                + (!isNaN(n) ? n : 35) + 'px; height: ' + r.Y + 'px; display:' + (r.n() <= r.display ? 'none' : 'block') + '">'
                + ' <span class="rT"></span><span class="rB" style="top:' + r.Y + 'px;"></span>'
                + ' <div id="' + i + 'Slide" class="slider" style="position: relative; height:' + o.height(i)
                + 'px; top:' + o.top(o, r) + 'px;">' + '  <div class="rT"></div><div class="rB" style="top:' + H + 'px"></div>'
                + ' </div>' + '</div>');
        },
...
//                            }
        ...
        'fi':function (s) {
            ...
            function fu(i,r)
            {
                ...
                if(i <= r.length)
                {
                    gui_filter.src[i] = new Image();
                    gui_filter.src[i].src =  'media/filter/' + vars.fi + '/' + (i - 1) + '-thumb.png';
                    gui_filter.src[i].id = i;
					var filtername = gui_filter.filterNameDic[vars['fi']][i];
                    return ('<canvas width="34" height="34" onmousedown="if(this.id.substr(2)!=gui_filter.fileNumber) '
                        + '{ gui_filter.current(this); }" id="fi' + i + '"  title="'+filtername+'"' 
                        + (gui_filter.fileNumber == i ? ' class="cur"' : '')
                        + ' onmouseup="gui_filter.buildfilterOp(vars[\'fi\'],this.id.substr(2))"'
                        + '></canvas>');
                }
            }
            if(r.active( $('fi'), 4, c[r.id], c[r.id]+ 11,{
                    'change': function(){gui_filter.src =[]},
                    'each': fu,
                    'vars': {'length': gui_filter.r[vars.fi]}
                },s))
            {
                for(var i in gui_filter.src)
                    gui_filter.src[i].onload = function(){gui_filter.preview(this.id);};
            }
        ...
        'stamp':function (s) {
            ...
            function fu(i, r) {
                if (i <= r.length) {
                    ...
                    stamp.src[i].id = i;
                    return ('<canvas width="34" height="34" onmousedown="if(this.id.substr(5)!=stamp.fileNumber) ' +
                        '{ stamp.current(this); co.glyph(stamp.uri(\'live\'),this.id); }" id="stamp' + i + '"'
                        + (stamp.fileNumber == i ? ' class="cur"' : '') + '></canvas>');
                }
            }
            ...
            if ($('brush_author')) {
                ...
                if (!stampSet) {
                    ...
                    $('brush_author').innerHTML = 
						//'<i style="-moz-user-select: none; -khtml-user-select: none; user-select: none; ">by:&nbsp; <a href="' + stampSet.url + '" target="_blank">' + stampSet.name + '</a></i>'+
    ...
    //* X-Slide 其中build函数最后加了一个参数h hooker的意思 为了控件 当mouseup事件发生时，挂载某个函数
    /**
     * 2012年8月7日11:43:44
     * build 运行程序的滑块有两个，所以在有了 if (!isNaN(r[0])) 这个判断。r这个对象存入了x的value [0,10,10,'float']
     * 现在先以r只有一个属性为例，for (var i in r) 遍历r，这个时候假设r只有一个属性。
     * 代码里a是用来判断滑块的位置的：vars[o + i] 是用户设定的值(也是初始值)，然后判断cF[o].val.[1]这个值是最大的量程，通过算这两个数的比值，来确定滑块的位置
     * 代码里b 是用来显示这个具体的值。
     *
     * html 根据这些值生成html，o id； r 值列表[0,10,10,'float'] 类似于这个；用户选的值和最大量程的比率；m事件属性
     *
     * 需要解决的问题是 需要选择的值是[-1,1]，目前支持的是范围都是[0,10] 都是从0开始，现在要在显示上动手脚，就是在原有的基础上加一个偏远值。
     */
    'X':{
...
//            if (r[3]) vars[o] = Math.round(Math.max(r[0], n * r[1]) * 100) / 100;
            ...
            if(cF[o] && cF[o].val[4]) {
                $(o + 'CurV').innerHTML =  Math.round((vars[o] - cF[o].val[4])*100)/100;
            }
            else
        ...
        'build':function (v, o, r, h) {
            ...
            for (var i in r) {
                n++;
              a = '<div id="' + o + i + 'Cur" class="' + i + '" style="left: '
                    + Math.round(vars[o + i] / (i ? cF[o].val[i] : cF[o].val)[1] * 112) + 'px"></div>' + a;
//                b = '<div id="' + o + i + 'CurV" class="v">' + Math.round(vars[o + i])
//                    + '</div>' + (b ? '<div class="u">/</div>' : '') + b;
                b = '<div id="' + o + i + 'CurV" class="v">' + Math.round(vars[o + i]-(cF[o].val[4]?cF[o].val[4]:0))
                                    + '</div>' + (b ? '<div class="u">/</div>' : '') + b;
            }

            return ('<div class="' + d + '"><span>' + v + '</span><br><div onmousedown="gui.X.'
//                + (n > 1 ? 'xxSwitch' : 'xRun') + '(\'' + o + '\',event,this)" class="slide_div">'
                + (n > 1 ? 'xxSwitch' : 'xRun') + '(\'' + o + '\',event,this)" class="slide_div'+((h)?' hooker':'')+'">'
                + (n > 1 ? '<div id="' + o + 'cCur" class="slide_center"></div>' : '') + a
                + '</div>' + b + '<br></div>' + (d == 'slideXX' ? '<br style="margin-top: 26px">' : ''));
        },
        'xRun':function (o, e, dom) { //dom代表this
            core.fu(o + 'Cur', e, {
                    fu:core.X,
                    oX:-6,
                    X1:0,
...
//                    gui.X.left(o, b.X);
                    ...
                    if (m == 'up'){
                        vars.cache(1);
                        if(dom.className.indexOf('hooker')> -1)
                            hooker.fi();
                    }
                });
        },
...
win = {
// in properties feed, have win.r restore the data which show in window
    // Mouse
    'close':function (v, s) {
        win.tog(v, {
            display:'none',
     ...
     * 页面各个控件的布局
     * @param v 控件id
     * @param s block or none 初始化是否显示该控件
     * @param r 一个数组，三个值
     */
    'tog':function (v, s, r) {
        if ($(v)) {
    ...
    'feed':function () {
        win.r = {
            canvas:['block', -10, 45, ],
            filter:['block', 810, 45, 256],
            solid:['block', 600, 45, 256],
            gradient:['none', 420, 570, 230],
            pattern:['none', 700, 570, 219],
            swatch:['none', 0, 570, 217],
            tools:['block', -140, 45, 279],
            options:['block', -140, 330, 200],
            history:['block', 600, 300, 187]
        };
        if (r = cookieGrab('windows'))
        {
            win.mk(r);
 //           console.info("cookieGrab('windows')");
        }
        else
        {
 //           console.info("!!  cookieGrab('windows')");
    ...
    'mk':function (r) {
        var n = win.getCenter(),
            r = r.split(':');
 //       console.info('win mk win.getCenter() '+ n);
        for (var i in r) {
            var v = r[i].split(','),
                o = '';
...
//            TEST("'" + v[0] + "':['" + v[1] + "'," + v[2] + "," + v[3] + "," + v[4] + "],", 1);
    ...
    'cp':function (o, r) {
		...
		if(o =='filter') win.r[o][3]='undefined';
        for (var i in win.r) {
            z += i + ',' + win.r[i] + ':';
        }
        if (o) {
...
//      canvas,block,-10,19:solid,block,588,49,256:gradient,none,338,382,230:pattern,none,338,152,219:swatch,none,589,306,217:tools,block,-102,19,279:options,block,-103,296,200:history,none,138,416,187:
...
win_size = {
    ...
    'fu':function (o, c) {
        ...
        c['BM'].width = o.W + 'px';
        var o = win_size.LT(),
            r = ['ctx_box', 'ctx_temp', 'ctx_marquee', 'ctx_active', 'ctx_mouse', 'ctx_filter'];
        for (var i in r) {
            var d = $S(r[i]);
            d.left = o.L + 'px';
            d.top = o.T + 'px';
        ...
        c['BM'].width = o.W + 'px';
        var olt = win_size.LT();
        var    r = ['ctx_box', 'ctx_temp', 'ctx_marquee', 'ctx_active', 'ctx_mouse', 'ctx_filter'];
        for (var i in r) {
          var d = $S(r[i]);
            d.left = olt.L + 'px';
          d.top = olt.T + 'px';
...
//        TEST('gui_tools.imageSize call',1);
        var o = $C(v, 'tools')[0],
            d = o.style;
        d.width = (15 + i) + 'px';
        d.height = (15 + i) + 'px';
...
gui_options = {
    modulesObject:function () {
        ...
        gui_options.modules = {
            ...
            'stamp':{
                build:['rand', 'br','flow_stamp', 'opacity_stamp'],
                vars:{
    ...
    forge:function (v) { // build "OPTIONS" window
        ...
        var r = gui_options.modules[v],   // options elements
            z = '';
        ...
        for (var i in r.build) {
            ...
            var o = i.indexOf('_') != -1 ? i.substr(0, i.indexOf('_')) : i,
                title = o.replace('_', ' ').replace('line', '').replace('rand', 'diameter min max')
                    .replace('font', '').replace('marq', '').replace('shape', '').toLowerCase(),
                fu = (i == 'br') ? '<br>' : gui[cF[i].type].build(title, i, cF[i].val);
//            console.info('title: '+title+'   raw: '+ i);
//            console.info('type: '+(i == 'br') ? '<br>':cF[i].type + '  cF[i].val:'+ cF[i].val);
            if (v == 'shape') {
                if ((d != 'ellipses' && d != 'polygon')
                    || (d == 'ellipses' && i != 'corner' && i != 'sides_shape' && i != 'slope_shape') ||
                    (d == 'polygon' && i != 'slope_shape')) {
        ...
        if (v == 'fill' || v == 'brush' || v == 'pencil' || v == 'calligraphy' || v == 'stamp') {
        // these tools use only the "fill" property
            gui_palette.click('fill')
        ...
        switch (v) { // extra stuff
            ...
            case 'stamp':
                z = '<div id="stamp"></div>' + gui.Y.kontrol('stamp') +
                    "<span style=\"position: relative; top: -6px; border: 0\" id=\"brush_author\"></span>" 
				+ z;
                break;
            case 'text':
                if (!vars.textMessage) {
        ...
        var d = $('options');
        $C('TML', d)[0].innerHTML = '<span style="-moz-user-select: none; -khtml-user-select: none; user-select: none; ">'
            + (r.head ? gui.menu.build(r.head, cF[r.head].val) : v) + '</div>';
//        console.info(r.head+'   '+ cF[r.head].val);
...
gui_color = {
    ...
    'mk':function () {
        ...
        for (var i in R) {
            ...
            z += '<div title="' + i.substr(0, 1).toUpperCase() + i.substr(1) + '">'
                + ' <div class="west" id="' + i + 'T">' + i.substr(0, 1).toUpperCase() + '</div>' +
                ' <span class="east" id="' + i + 'Me"></span>'
                + ' <div onmousedown="' + v + '; return false;" id="' + i + 'Cur" class="cur"></div>' +
...
gui_gradient = {
    ...
    'mk':function () {
        ...
        o.innerHTML = '<div class="z" onmousedown="gui_swatch.toType(\'GD\')">'
            + ' <div onmousedown="core.fu(\'cGD1\',event, {fu:gui_gradient.slide_o, oX:-7, oY:-13}); return false;" id="gAngle" class="blue_dot" title="Angle"></div>'
            + ' <div onmousedown="core.fu(\'cGD1\',event, {fu:core.Y, oX:0, oY:-41, Y1:-5, Y2:86}, gui_gradient.slide_y); return false;" class="blue_slide"><div id="gOpacity" class="blue_dot" title="Opacity"></div></div>'
            + ' <div id="gPos"></div>'
            + ' <canvas id="cGD1" height="169" width="169"></canvas>'
            + ' <canvas id="cGD2" onmousedown="core.fu(\'cGD1\',event, {fu:gui_gradient.slide_o, oX:-7, oY:-13})" height="169" width="169"></canvas>'
            + ' <div class="slide_x" onmousedown="gui_gradient.add(event)"></div>' + '</div>';
        gui_gradient.mk_x();
        infc.info_draw('cGD2');
    ...
    'mk_x':function (o) {
        var g = vars[vars.id + 'GD'],
            z = '';
        for (var i in g) z += '<div onmousedown="gui_gradient.cur(this); gui_gradient.cs(); core.fu(\'gd'
            + i + '\',event,{fu:core.X, oX:0, oY:15, X1:0, X2:169}, gui_gradient.slide_x)" id="gd' + i + '" style="left: '
            + ((g[i][0] * 169) - 5) + 'px"><canvas height="7" width="7"></canvas></div>';
...
var gui_pattern = {
    ...
    'mk':function (s) {
        var j = 0,
            ...
            o = $C('MM', 'pattern')[0],
            z = '';
        o.innerHTML = '<div class="z" onmousedown="gui_swatch.toType(\'PT\')">'
            + ' <div onmousedown="core.fu(\'cPT1\',event, {fu:core.Y, oX:0, oY:-41, Y1:-5, Y2:86}, gui_pattern.slide_y); return false;" class="blue_slide"><div id="pOpacity" class="blue_dot" title="Opacity"></div></div>'
            + ' <div id="pPos"></div>'
            + ' <canvas id="cPT"></canvas>' + ' <canvas id="cPT1" height="169" width="169"></canvas>' + ' <canvas id="cPT2" height="169" width="169"></canvas>' + '</div>';
...
gui_swatch = {
    ...
    'mk':function () {
        ...
        for (var i in gui_swatch.r) {
            ...
            gui_swatch.set[i] = {};
            for (var j in Q[i])
                gui_swatch.set[i][k++] = j;
        }
        for (var i in gui_swatch.r) {

            var res = '<div id="author_' + i + '" class="author">' + 
				gui_swatch.author(i) 
				+ '</div>';
            z += '<div class="menu ' + i + 'menu" onmousedown="gui_swatch.cur(\'' + i + '\'); vars.cache(1);">'
                + ' <div>' + gui_swatch.r[i][0].toUpperCase() + 'S</div>' +
                ' <span class="east">' +
...
//				 '  <img src="media/gui/sw_add.png" onmousedown="if(this.parentNode.parentNode.style.cursor==\'default\') gui_swatch.add(); return false;" alt="...">' +
                ...
                ' <div style="position: absolute; left: 19px; top: ' + (42 + gui_swatch.r[i][2]) + 'px;">'
                + gui.menu.build(i + '*', gui_swatch.set[i]) + '</div>' +
                ' <div id="' + i + '" class="squares"></div>' + gui.Y.kontrol(i, gui_swatch.r[i][2] + 49) + '<br>' +
//					res +
                '</div>';
        }
...
(function () {
    ...
    document.onkeydown = function (e) {
        ...
        else if (e.ctrlKey && E.sh && k == 90 && (r = canvas.history_r) && r.r[r.n + 1] <= canvas.history_n
            && (r.n + 1) <= r.z) { // REDO
            canvas.history_set(r.n++);
        } else if (e.ctrlKey && !E.sh && k == 90 && (r = canvas.history_r)
            && r.r[r.n - 1] >= 0 && (r.n - 1) >= r.a) { // UNDO
            canvas.history_set(r.n--);
...
/* MARQUEE
* 当用户选择这个工具绘制选区的时候，会把用户的绘制的结果保存在 marquee.shapes 这个数据里，同时设置marquee.on这个属性
* 在用户选择其他工具进行操作的时候，mask_active，这个方法会把 marquee.shapes 里存储的图形绘制到实际操作的canvas上，然后clip
* */
...
//            console.info("m != 'down' mouse.id:"+ mouse.id+" mouse.moved:"+ mouse.moved +" mouse.down:"+mouse.down);
            if (!mouse.down && e) {
                marquee.reset(b);
                mouse.down = 1;
                if (marquee.shape > 0) {
  ...
  //          console.info('m == "down" && E.sh_var && marquee.shapes.length');
            marquee.dash(ants[ants_n]);
        } else {
 //           console.info("marquee default");
            marquee.reset();
        }
    },
    /**
     * marquee的绘制 绘制在c 代表的canvas,本方法本身不指定canvas
     * 这个方法会访问marquee对象的shapes
     * @param c
     * @param s1
     * @param s2
     */
    ...
    /**
     * marquee最终实现的效果，是通过在 ctx_marquee  canvas画布上绘制图案，然后填充相关的style
     * @param p
     * @param c
     * @param s
     */
    'dash':function (p, c, s) {
 //       console.info('marquee dash call');
...
crop = {
    ...
    'move':function (x) {
        ...
        var value = function (a, v) {
            return({X:a.X + (!isNaN(v) ? v : v.X), Y:a.Y + (!isNaN(v) ? v : v.Y)});
        };
    ...
    'click':function () {
        if (vars.type == 'crop') {
            ...
            if (crop.force(vars.crop) || !v) {
                ...
                gui.menu.constrain = String(vars.aspect);
                ...
                if (o.constrain) {
                    vars.aspect = String(o.constrain);
                    o.constrain = '';
                }

                $T('div', $('aspect_radio'))[vars.aspect == 'portrait' ? 1 : 0].className = 'cur';
                $S('aspect_radio').opacity = 1;

            }

            if (!isNaN(aXY.X) && !isNaN(bXY.X)) {

                if (vars.constrain == 'true') {
                    crop.aspect(aXY, bXY);
                    crop.transform(aXY, bXY);
                }
...
//                console.info("crop up");
    ...
    'transform':function (a, b) {
        var r = crop.ratio[gui.menu.key.crop], R2 = r[0] / r[1];
    ...
    'XY':function (a, b, r) {
        var c = {X:((b.X - a.X) * .5) + a.X, Y:((b.Y - a.Y) * .5) + a.Y};

        a.X = Math.round(c.X - r.X);
        a.Y = Math.round(c.Y - r.Y);
        b.X = Math.round(c.X + r.X);
        b.Y = Math.round(c.Y + r.Y);
    ...
    'apply':function (a, b) {
        var c = $2D('ctx_temp'), prev = {W:canvas.W, H:canvas.H};
    ...
    'resize':function () {
        ...
        $S('cBound').cursor = 'crosshair';
        mouse.id = '';
        aXY = {};
        bXY = {};
        mouse.area = '';

        var r = ['ctx_temp', 'ctx_marquee', 'ctx_active', 'ctx_mouse','ctx_filter'];
...
transform = function (a, b, m, e) {
    ...
    if (e) {
        ...
        function zarea(v) {
            transXY = {X:0, Y:0};
            $S('cBound').cursor = mouse.id = v;
            co.core(e);
        }

        if (mouse.id == 'n-resize') {
            a.X = aXY.X;
            a.Y = bXY.Y;
            b.X = bXY.X;
            b.Y = Math.max(0, aXY.Y + (b.Y - oXY.Y));

            if (a.Y < b.Y) {
                var v = bXY;
                bXY = {X:bXY.X, Y:aXY.Y - o.Y};
                aXY = {X:aXY.X, Y:v.Y};
                zarea('s-resize');
            }

        }
        else if (mouse.id == 'ne-resize') {
            a.X = aXY.X;
            a.Y = bXY.Y;
            b.X = Math.min(canvas.W, bXY.X + (b.X - oXY.X));
            b.Y = Math.max(0, aXY.Y + (b.Y - oXY.Y));

            if (a.Y < b.Y) {
                var v = bXY;
                bXY = {X:bXY.X - o.X, Y:aXY.Y - o.Y};
                aXY = {X:aXY.X, Y:v.Y};
                zarea('se-resize');
            }
                ...
                var v = bXY;
                bXY = {X:aXY.X, Y:bXY.Y};
                aXY = {X:v.X - o.X, Y:aXY.Y - o.Y};
                zarea('nw-resize');
            }

        }
        else if (mouse.id == 'e-resize') {
            a.X = aXY.X;
            a.Y = aXY.Y;
            b.X = Math.min(canvas.W, bXY.X + (b.X - oXY.X));
            b.Y = bXY.Y;
            ...
            if (a.X > b.X) {
                var v = bXY;
                bXY = {X:aXY.X, Y:bXY.Y};
                aXY = {X:v.X - o.X, Y:aXY.Y};
                zarea('w-resize');
            }

        }
        else if (mouse.id == 'se-resize') {
            a.X = aXY.X;
            a.Y = aXY.Y;
            b.X = Math.min(canvas.W, bXY.X + (b.X - oXY.X));
            b.Y = Math.min(canvas.H, bXY.Y + (b.Y - oXY.Y));

            if (a.Y > b.Y) {
                var v = bXY;
                bXY = {X:bXY.X, Y:aXY.Y};
                aXY = {X:aXY.X, Y:v.Y - o.Y};
                zarea('ne-resize');
            }
        ...
        else if (mouse.id == 'sw-resize') {
            a.X = bXY.X;
            a.Y = aXY.Y;
            b.X = Math.max(0, aXY.X + (b.X - oXY.X));
            b.Y = Math.min(canvas.H, bXY.Y + (b.Y - oXY.Y));

            if (a.Y > b.Y) {
                var v = bXY;
                bXY = {X:bXY.X, Y:aXY.Y};
                aXY = {X:aXY.X - o.X, Y:v.Y - o.Y};
                zarea('nw-resize');
            }

            else if (a.X < b.X) {
                var v = bXY;
                bXY = {X:aXY.X - o.X, Y:bXY.Y - o.Y};
                aXY = {X:v.X, Y:aXY.Y};
                zarea('se-resize');
            }

        }
        else if (mouse.id == 'w-resize') {
            a.X = bXY.X;
            a.Y = aXY.Y;
            b.X = Math.max(0, aXY.X + (b.X - oXY.X));
            b.Y = bXY.Y;
            ...
            if (a.X < b.X) {
                var v = bXY;
                bXY = {X:aXY.X - o.X, Y:bXY.Y};
                aXY = {X:v.X, Y:aXY.Y};
                zarea('e-resize');
            }

        }
        else if (mouse.id == 'nw-resize') {
            a.X = bXY.X;
            a.Y = bXY.Y;
            b.X = Math.max(0, aXY.X + (b.X - oXY.X));
            b.Y = Math.max(0, aXY.Y + (b.Y - oXY.Y));
            ...
            b.X = Math.min(canvas.W, Math.max(0, b.X));
            b.Y = Math.min(canvas.H, Math.max(0, b.Y));
    ...
    if (m == 'down') {
        transXY = {X:b.X - oXY.X, Y:b.Y - oXY.Y};
    }
...
// helper functions
    ...
    /**
     * 找到canvas ctx_temp，清空这个canvas, 并最终返回这个canvas
...
//        c.clearRect(0, 0, canvas.W, canvas.H);
 ...
 //           console.info('clear ctx_temp');
    ...
    ;
    /**
     * 得到 ctx_temp canvas，将结果拷贝到 ctx_box 这个canvas上
     * 并且清空  ctx_temp
     * @param c
     */
    function mask_up(c) {
//        console.info('mask_up');
...
Q.GD['Web v2.1']=[
	...
	[[0,[242,130,91,1]],[0.5,[229,91,43,1]],[1,[240,113,70,1]]],  	[[0,[255,48,25,1]],[1,[207,4,4,1]]],  	[[0,[255,26,0,1]],[1,[255,26,0,1]]],  	[[0,[204,0,0,1]],[1,[204,0,0,1]]],   	[[0,[248,80,50,1]],[0.5,[241,111,92,1]],[0.5,[246,41,12,1]],[1,[231,56,39,1]]],	[[0,[254,204,177,1]],[0.5,[241,116,50,1]],[0.5,[234,85,7,1]],[1,[251,149,94,1]]], 	[[0,[239,197,202,1]],[0.5,[210,75,90,1]],[0.5,[186,39,55,1]],[1,[241,142,153,1]]],	[[0,[243,197,189,1]],[0.5,[232,108,87,1]],[0.5,[234,40,3,1]],[0.75,[255,102,0,1]],[1,[199,34,0,1]]],
	[[0,[183,222,237,1]],[0.5,[113,206,239,1]],[0.5,[33,180,226,1]],[1,[183,222,237,1]]], 	[[0,[224,243,250,1]],[0.5,[216,240,252,1]],[0.5,[184,226,246,1]],[1,[182,223,253,1]]],	[[0,[254,255,232,1]],[1,[214,219,191,1]]],	[[0,[252,255,244,1]],[1,[233,233,206,1]]],	[[0,[252,255,244,1]],[1,[179,190,173,1]]], 	[[0,[229,230,150,1]],[1,[209,211,96,1]]],
	[[0,[234,239,181,1]],[1,[225,233,160,1]]],  	[[0,[69,72,77,1]],[1,[0,0,0,1]]],    	[[0,[125,126,125,1]],[1,[14,14,14,1]]],  	[[0,[149,149,149,1]],[0.5,[1,1,1,1]],[0.76,[78,78,78,1]],[1,[27,27,27,1]]],
	[[0,[174,188,191,1]],[0.5,[110,119,116,1]],[0.5,[10,14,10,1]],[1,[10,8,9,1]]],
	[[0,[197,222,234,1]],[1,[6,109,171,1]]],   	[[0,[247,251,252,1]],[1,[173,217,228,1]]],	[[0,[214,249,255,1]],[1,[158,232,250,1]]], 	[[0,[233,246,253,1]],[1,[211,238,251,1]]], 	[[0,[99,182,219,1]],[1,[48,157,207,1]]],    	[[0,[44,83,158,1]],[1,[44,83,158,1]]],      	[[0,[169,228,247,1]],[1,[15,180,231,1]]],   	[[0,[178,225,255,1]],[1,[102,182,252,1]]],  	[[0,[79,133,187,1]],[1,[79,133,187,1]]],   	[[0,[147,206,222,1]],[1,[73,165,191,1]]],  	[[0,[222,239,255,1]],[1,[152,190,222,1]]],  	[[0,[73,192,240,1]],[1,[44,175,227,1]]],   	[[0,[254,255,255,1]],[1,[210,235,249,1]]],    	[[0,[167,207,223,1]],[1,[35,83,138,1]]],   	[[0,[73,155,234,1]],[1,[32,124,229,1]]],   	[[0,[53,106,160,1]],[1,[53,106,160,1]]],   	[[0,[255,255,255,1]],[1,[237,237,237,1]]],	[[0,[242,249,254,1]],[1,[214,240,253,1]]],	[[0,[255,255,255,1]],[1,[229,229,229,1]]],	[[0,[255,255,255,1]],[0.5,[241,241,241,1]],[0.5,[225,225,225,1]],[1,[246,246,246,1]]],	[[0,[255,255,255,1]],[0.5,[243,243,243,1]],[0.5,[237,237,237,1]],[1,[255,255,255,1]]],	[[0,[246,248,249,1]],[0.5,[229,235,238,1]],[0.5,[215,222,227,1]],[1,[245,247,249,1]]],	[[0,[246,230,180,1]],[1,[237,144,23,1]]], 	[[0,[234,185,45,1]],[1,[199,152,16,1]]],	[[0,[255,214,94,1]],[1,[254,191,4,1]]],
	[[0,[241,231,103,1]],[1,[254,182,69,1]]], 	[[0,[255,255,136,1]],[1,[255,255,136,1]]], 	[[0,[254,191,1,1]],[1,[254,191,1,1]]]];
...
canvas = {
    ...
    'history_set':function (v, s,filter) {
        if ($('history_0')) {
            var o = gui.Y,
                r = canvas.history_r;
            if (isNaN(v)) { // SAVE
				...
				if(filter)
					r.data[r.n] = [canvas.W, canvas.H, filter[0], filter[1], ];
				else
...
//    'load':function (u, w, h) {
//
//        console.info('canvas load is call @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@');
//        gui_swatch.id = 'CO';
        ...
        o.onmousemove = function (e) {
            if (stop) {
                ...
                if (vars.type == 'picker') {
                    ...
                    a.X -= abPos(this).X;
                    a.Y -= abPos(this).Y;
                    a.X = Math.max(0, Math.min((canvas.W)?canvas.W:vars.winW - 1, a.X));
                    a.Y = Math.max(0, Math.min((canvas.H)?canvas.H:vars.winH - 1, a.Y));
                    picker.core(a, '', 'move');
                }
            }
        };
        ...
        Event.add(canvasdiv,{
            ...
            function(e){
                ...
                filereader.onload = function(e){
					console.info('reader onload call');
					var img = new Image();
                ...
                filereader.onerror = function(e){
                    ...
                    if(code == 2){
                        alert('MUST RUN in HTTP');
                    }else{
                        alert('error code: ' + code);
                    }
                };
				filereader.readAsDataURL(file);
                console.info('reader data success');
            });
    },
...
//        console.log(width, height);
		canvas.W=width;
		canvas.H=height;
		win_size.fu2({W:width, H:height}, win_size.construct({}));
   }
...
window.preview= function(){
	var url = $('ctx_box').toDataURL("image/png"); 
	w = window.open('previewonlocal.html'); 
    w.onload = function(){
        var placeholder = w.document.getElementById("img");  
        placeholder.src = url;
    };  
}
...
window.saveDrawing = function () {
	...
	*/
	var canvasData = $('ctx_box').toDataURL("image/png");
	var postData = "imageData="+canvasData;
...
//			fileinfo.innerHTML="Saved as<br><a target='_blank' href='"+ajax.responseText+"'>"+ajax.responseText	+"</a>";
			fileinfo.innerHTML = ajax.responseText;
			if(fileinfo.innerHTML.trim().length>0){//share icon change green
				$('shareicon').style.backgroundImage="url(media/gui/share.png)"
			}
			else{//share icon change gray
			}
		} 
	}
...
window.share=function(){
	...
	if(fileinfo.innerHTML.trim().length>0)
	{
		url = $('fileinfo').innerHTML;
//		console.info(url);
		w = window.open('shareimage.html');
		w.onload = function(){
			var placeholder = w.document.getElementById("img");  
			placeholder.src = url;
		};
...
window.testsaveDrawing = function(canvasname)
{
	
    if($(canvasname))
        window.open($(canvasname).toDataURL());
}
